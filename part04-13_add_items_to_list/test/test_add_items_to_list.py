    ...
    def test_inputs_1(self):
        ...
        with patch("builtins.input", side_effect=[str(x) for x in values]):
            ...
            self.assertEqual(
                len(output_list),
                1,
                f"In addition to asking for the inputs from the user, your program should print out one row, now it prints out {len(output_list)} rows, when the input is: {values}",
            )
