...
class ETjaVSanaTest(unittest.TestCase):
    ...
    def test_5_second_word_function_ok(self):
        for row in [
            ...
            with patch("builtins.input", side_effect=["2 2"] * 10):
                ...
                self.assertFalse(
                    len(output_all) > 0,
                    f'Calling second_word("{row}") should not print out anything, but it prints out\n{output_all}\nremove print commands inside function',
                )
    ...
    def test_6_last_word_exists(self):
        try:
            ...
            self.assertTrue(
                False, f"Your code should contain function named as last_word"
            )
        try:
            from src.first_second_last import last_word
            ...
            self.assertTrue(
                False,
                f'Make sure, that function can be called as follows:\last_word("once upon a time there was a programmer")',
            )
    ...
    def test_7_last_word_function_ok(self):
        for row in [
            ...
            with patch("builtins.input", side_effect=["2 2"] * 10):
                ...
                self.assertFalse(
                    res == None,
                    f'Calling last_word("{row}") should return\n{odotettu}\nnow it does not return anything. Make sure that you use return command in any cases in your function!',
                )
                self.assertEqual(
                    res,
                    odotettu,
                ...
                self.assertFalse(
                    len(output_all) > 0,
                    f'Calling last_word("{row}") should not print out anything, but it prints out\n{output_all}\nremove print commands inside function',
                )
