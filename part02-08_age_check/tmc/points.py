...
def points(*points):
    ...
    def points_wrapper(o):
        if isclass(o):
            ...
            raise Exception(
                "Expected decorator object '%s' type to be Class or Function but was %s."
                % (o, type(o))
            )
        return o
    ...
    if not points:
        raise Exception(
            "You need to define at least one point in the points decorator declaration"
        )
    for point in points:
        if type(point) is not str:
            msg = "Points decorator argument '%s' needs to be a string, but was %s." % (
                point,
                type(point).__name__,
            )
            raise Exception(msg)
