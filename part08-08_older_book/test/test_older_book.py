...
"""
    ...
    def test1_function_exists(self):
        try:
            ...
            self.fail(
                "Your program should contain function older_book(book1: Book, book2: Book)"
            )
    ...
    def test2_return_type(self):
        try:
            ...
            val = older_book(
                Book("Python", "P. Python", "non-fiction", 2000),
                Book("Java", "J.Java", "non-fiction", 2001),
            )
            taip = str(type(val)).replace("<class '", "").replace("'>", "")
            self.assertTrue(
                val == None,
                f"Function older_book should not return a value,"
            ...
            self.fail(
                f"Function threw an error when called with argument values\n"
    ...
    def test3_test_first_older(self):
        test_cases = (
            (
                ("Seven Brothers", "Aleksis Kivi", "Novel", 1870),
        ...
        for test_case in test_cases:
            with patch(
                ...
                self.assertTrue(
                    "older" in output
                    and older.name in output
                    and not_older.name not in output
                    and str(older.year) in output
                    and str(not_older.year) not in output,
    ...
    def test4_test_second_older(self):
        test_cases = (
            (
                ("The Eigth Brother", "Alex Stone", "Novel", 1970),
                ("The Egyptian", "Mika Waltari", "Novel", 1945),
            ),
            (
    ...
    def test5_test_equally_old(self):
        test_cases = (
            (
                ("The Ninth Brother", "Alex Stone", "Novel", 1980),
        ...
        for test_case in test_cases:
            with patch(
                "builtins.input",
                side_effect=[AssertionError("Syötteen pyytämistä ei odotettu")],
            ):
                reload_module(self.module)
                older_book = load(exercise, function, "en")
                from src.older_book import Book
                ...
                self.assertTrue(
                    "and" in output
                    and book1.name in output
                    and book2.name in output
                    and str(book1.year) in output,
                    f"Program output\n{output}\nis different to model answer\n{corr}\nwhen books are\n{test_case}",
                )
