...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as longest(strings: list)",
            )
        try:
            from src.longest_string import longest
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == str,
            f'Function {function} does not return value of string type when calling longest(["ab","a"])',
        )
        ...
        for tc in test_cases:
            ...
            with patch(
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure, that the function works when the list is\n{test_case}",
                    )

                self.assertEqual(
                    correct,
                    test_result,
                    f"The result '{test_result}' does not match with the model solution '{correct}' when the list is {test_case}",
                )
