...
class NumbesrStatsTest(unittest.TestCase):
    ...
    def test2_constructor(self):
        with patch("builtins.input", side_effect=["0", "-1"]):
            try:
                ...
                self.fail(
                    "Calling constructor NumberStats() " + f" threw an error: {e}"
                )
    ...
    def test2b_test_methods(self):
        ...
        try:
            ...
            self.fail(
                f"Calling method add_number(1) threw an error {e}, "
        ...
        try:
            ...
            self.fail(
                f"Calling method count_numbers() threw an error {e}, "
                + "Check that the method is included in the class!"
            )
        ...
        for test_case in test_cases:
            with patch("builtins.input", side_effect=["0", "-1"]):
                try:
                    ...
                    self.fail(
                        f"Using class threw an error:\n{e}"
                        + "\nObject was initialized with\n"
                        + f"NumberStats()\nand method add_number called with values {test_case}"
                    )

                self.assertEqual(
                    number_stats.count_numbers(),
                    corr,
                    f"The number count should be {corr}, when object was initialized with\n"
    ...
    def test3_test_get_sum(self):
        ...
        try:
            ...
            self.fail(
                f"Method call get_sum() threw an error {e}, "
        ...
        for test_case in test_cases:
            with patch("builtins.input", side_effect=["0", "-1"]):
                ...
                self.assertEqual(
                    val,
                    corr,
                    f"The sum on numbers should be {corr}, when object was initialized as\n"
                ...
                self.assertEqual(
                    number_stats.count_numbers(),
                    len(test_case),
                    f"Count should be {len(test_case)}, when object was initialized as\n"
                    + f"NumberStats()\nand method add_number was called with {test_case}.\n"
                    + f"Now count_numbers returns {number_stats.count_numbers()}.",
                )
    ...
    def test3_test_empty_average(self):
        try:
            ...
            self.assertTrue(
                False,
                "Make sure, that you do not divide by zero, when calling method average() with no numbers added.",
            )
        except Exception as e:
            self.assertTrue(
                False,
                f"Calling method average() with no numbers added threw an error:\n{e}",
            )
    ...
    def test3_test_average(self):
        ...
        try:
            ...
            self.fail(
                f"Calling method average() threw an error {e}, "
        ...
        for test_case in test_cases:
            ...
            with patch("builtins.input", side_effect=["0", "-1"]):
                ...
                self.assertEqual(
                    val,
                    corr,
                    f"The average of numbers should be {corr}, when object was initialized with\n"
                    + f"NumberStats()\nand method add_number was called with {test_case}.\n"
