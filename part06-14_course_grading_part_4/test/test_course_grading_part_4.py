...
class CourseGradingPart4Test(unittest.TestCase):
    ...
    def test_1_works_with_files_1(self):
        ...
        with patch(
            "builtins.input", side_effect=words + [AssertionError("Too many inputs.")]
        ):
            try:
                ...
                self.assertTrue(
                    False,
                    f"Ensure that your program can be executed with input\n{f(words)}",
                )
...
"""
            ...
            self.fail(
                f"Your program should create file results.txt with imput\n{f(words)}\n{mssage}"
            )
        ...
        for i in range(3):
            ...
            self.assertEqual(
                line.strip(),
                expRows[i],
                f"File results.txt is not correct with input\n{f(words)}\nLine:\n{line}\nis not what was exepcted\nIt should be:\n{exp}",
            )
        ...
        try:
            ...
            self.assertTrue(
                False,
                f"Your program should create a file results.csv with input\n{f(words)}",
            )
