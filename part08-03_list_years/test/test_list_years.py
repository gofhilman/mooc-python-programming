...
"""
    ...
    def test2_return_type(self):
        try:
            ...
            self.assertTrue(
                type(val) == list,
                f"Function list_years should return a value of type list,"
                + f"now it returns {val} which is of type {taip}\n when it's called with arguments \n[date(1900,1,1)]",
            )
        except Exception as e:
            self.fail(
                f"Function threw an error when called with arguments [date(1900,1,1)]"
            )

    def test3_test_values(self):
        d = date
        test_cases = (
            [d(1900, 1, 1), d(1950, 2, 3), d(1979, 6, 6)],
            [d(2010, 5, 11), d(2009, 11, 1), d(2004, 3, 3), d(2000, 1, 23)],
            [d(1976, 8, 8), d(1984, 12, 24), d(1979, 2, 4), d(1980, 9, 3)],
            [d(1763, 2, 7), d(1454, 11, 11), d(1133, 2, 23), d(1755, 4, 22)],
        )
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    test_case,
                    test_case_2,
                    f"Function should not change the list\nList is now \n{test_case}, \nbut it should be \n{test_case_2}",
                )
