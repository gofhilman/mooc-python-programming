    ...
    def test_1_first_one_comes_last(self):
        ...
        for valuegroup in values:
            with patch("builtins.input", side_effect=list(valuegroup)):
                ...
                self.assertTrue(
                    len(output) == 1,
                    "After asking the inputs from user, instead of one row, your program's output is in {} rows: {} with input:\n{}".format(
                        len(output), output, format_tuple(valuegroup)
                    ),
                )
                self.assertTrue(
                    output[0].find(valuegroup[0]) > -1,
                    "From the output {} alphabetically last word {} is not found when input is {}".format(
                        output[0], valuegroup[0], format_tuple(valuegroup)
                    ),
                )
    ...
    def test_2_second_one_comes_last(self):
        ...
        for valuegroup in values:
            with patch("builtins.input", side_effect=list(valuegroup)):
                ...
                self.assertTrue(
                    output[0].find(valuegroup[1]) > -1,
                    "From the output {} alphabetically last word {} is not found when input is {}".format(
                        output[0], valuegroup[1], format_tuple(valuegroup)
                    ),
                )
    ...
    def test_3_equal_ones(self):
        ...
        for valuegroup in values:
            with patch("builtins.input", side_effect=list(valuegroup)):
                ...
                self.assertTrue(
                    output[0].find("same word twice") > -1,
                    "Output {} does not contain mention 'You gave the same word twice' when input is {}".format(
                        output[0], format_tuple(valuegroup)
                    ),
                )
