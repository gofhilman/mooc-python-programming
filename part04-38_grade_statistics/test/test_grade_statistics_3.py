...
Grade distribution:
  ...
  0: **""".split(
        ...
        with patch(
            ...
            self.assertTrue(
                len(output_all) > 0,
                f"Your program does not printout anything with the input\n{f(words)}",
            )
            output = [line for line in output_all.split("\n") if len(line) > 0]
            self.assertEqual(
                len(expected),
                len(output),
                f"Instead of 10 rows, your program output is now in {len(output)} rows:\n{output_all}\nwith the input:\n{f(words)}",
            )
            for i in range(len(expected)):
                ...
                self.assertEqual(
                    line,
                    e,
                    f"The print out in row {i+1} is incorrect, it should be\n{e}\nbut row is\n{line}\nthe whole print out is:\n{output_all}\nwith the input:\n{f(words)}",
                )
