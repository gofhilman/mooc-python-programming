...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as play_turn(game_board: list, x: int, y: int, piece: str)",
            )
        try:
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == bool,
            f"Function {function} does not return boolean type with parameter values [['','',''],['','',''],['','','']], 0, 0, 'X'.",
        )
    ...
    def test_3_free_squares(self):
        ...
        for test_case in test_cases:
            with patch(
                ...
                test_result = play_turn(
                    test_board, test_case[0], test_case[1], test_case[2]
                )

                self.assertEqual(
                    correct,
                    test_result,
                    f"The result {test_result} does not match with the model solution {correct} when the parameters are \n{test_board2}, {test_case[0]}, {test_case[1]}, {test_case[2]}",
                )
                self.assertEqual(
                    test_board,
                    board_after,
                    f"State of the matrice\n{test_board}\ndoes not match with the model solution\n{board_after}\nwhen the parameters are \n{test_board2}, {test_case[0]}, {test_case[1]}, {test_case[2]}",
                )
        ...
        for test_case in test_cases:
            with patch(
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure, that you can call the function with following parameters\n{test_board2}, {test_case[0]}, {test_case[1]}, {test_case[2]}",
                    )
        ...
        for test_case in test_cases:
            with patch(
                ...
                try:
                    ...
                    self.assertFalse(
                        True,
                        f"Make sure that the function can be called with following parameters\n{test_board2}, {test_case[0]}, {test_case[1]}, {test_case[2]}",
                    )
