        ...
        for testcase in words:
            with patch("builtins.input", side_effect=testcase.split(",")):
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"The execution of your program fails with the input {testcase}",
                    )
                ...
                self.assertTrue(
                    len(output) == cor_len,
                    f"Instead of {cor_len} rows, your program prints out with the input ({testcase}) {len(output)} rows. The correct print out is\n{correct}",
                )
                self.assertTrue(
    ...
    def test_words_3(self):
        ...
        for testcase in words:
            with patch("builtins.input", side_effect=testcase.split(",")):
                ...
                self.assertTrue(
                    len(output) == 0,
                    f"Your program should not print out anything with the input {testcase}\nbut it printed out\n{output_all}",
                )
