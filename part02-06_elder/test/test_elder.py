        ...
        for valuegroup in values:
            with patch("builtins.input", side_effect=list(valuegroup)):
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure that your program works with input\n{format_tuple(valuegroup)}",
                    )
                    pass
                ...
                self.assertFalse(
                    len(output) == 0,
                    "Your program does not print out anything with input:\n{}".format(
                        format_tuple(valuegroup)
                    ),
                )
                self.assertTrue(
                    ...
                    "After asking the inputs from user, instead of one row, your program's output is in {} rows: {} with input:\n{}".format(
                        len(output), p(output), format_tuple(valuegroup)
                    ),
                )
                self.assertTrue(
                ...
                self.assertFalse(
                    output[0].find(valuegroup[2]) > -1,
                    "From output\n{}\nthe name of the younger person was found {} when input was:\n{}".format(
                        output[0], valuegroup[2], format_tuple(valuegroup)
                    ),
                )
