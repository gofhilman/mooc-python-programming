...
class VsCodeTest(unittest.TestCase):
    ...
    def test_1_program_stops(self):
        ...
        with patch(
            "builtins.input",
            side_effect=words + [AssertionError("Input is asked too many times.")],
        ):
            try:
                ...
                self.assertTrue(
                    False,
                    f"Make sure that the execution of the program ends with the input\n{f(words)}",
                )

    def test_2_functionality(self):
        for string in [
            "emacs;visual studio code",
            "word;emacs;notepad;visual studio code",
        ]:
            words = string.split(";")
            ...
            with patch(
                ...
                self.assertEqual(
                    len(words),
                    len(output),
                    f"Instead of {len(words)} rows, your programs print out is in {len(output)} rows:\n{output_all}\nwith the input:\n{f(words)}",
                )
                for i in range(len(words) - 2):
                    ...
                    self.assertEqual(
                        line,
                        e,
                        f"Row {i+1} in your programs print out is incorrect, it should be\n{e}\nrow is\n{line}\nwith the input:\n{f(words)}\nthe print out is:\n{output_all}",
                    )
                ...
                self.assertEqual(
                    line,
                    e,
                    f"Last row of print out of your program is incorrect, it should be\n{e}\nrow is\n{line}\nwith the input:\n{f(words)}\the print out is:\n{output_all}",
                )
