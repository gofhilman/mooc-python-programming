...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as invert(dictionary: dict)",
            )
        try:
    ...
    def test_2_type_of_return_value(self):
        ...
        try:
            ...
            self.assertTrue(
                False,
                f"Make sure, that following function call works: invert({test_case})",
            )

        self.assertTrue(
            val == None,
            f"Function {function} should not return anything, now it returns value which is {type(val)} type.",
        )
    ...
    def test_3_invert(self):
        test_cases = (
            ...
            {x: x * 100 for x in range(1, 10)},
            {x: x - 1 for x in range(10, 90, 10)},
        )
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    len(correct),
                    len(test_case),
                    f"The returned dictionary should contain {len(correct)} items, but it contains {len(test_case)} items: \n{test_case}\nwhen calling invert({test_case2})",
                )
                self.assertEqual(
                    test_case,
                    correct,
