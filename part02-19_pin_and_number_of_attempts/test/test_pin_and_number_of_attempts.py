...
class PinTest(unittest.TestCase):
    ...
    def test_1(self):
        with patch(
            "builtins.input",
            side_effect=["4321", AssertionError("Input is asked too many times.")],
        ) as prompt:
            reload_module(self.module)
            output = get_stdout()
    ...
    def test_moar(self):
        for wrongs in [3, 4, 7, 11, 23]:
            ...
            with patch(
                "builtins.input",
                side_effect=["0000"] * wrongs
                + ["4321", AssertionError("Input is asked too many times.")],
            ) as prompt:
                reload_module(self.module)
                output = get_stdout()
                ...
                expected = "\n".join(
                    ["Wrong"] * wrongs
                    + ["Correct! It took you " + str(wrongs + 1) + " attempts"]
                )
                self.assertEqual(
                    sanitize(expected),
                    sanitize(output),
