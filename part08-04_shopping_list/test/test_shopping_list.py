...
"""
    ...
    def test1_function_exists(self):
        try:
            ...
            self.fail(
                "Your program must contain a function total_units(my_list: ShoppingList)"
            )
    ...
    def test2_return_type(self):
        try:
            ...
            self.assertTrue(
                type(val) == int,
                f"Function total_units should return an object of type int,"
                + f" now it returns {val} which is of type {taip}\nwhen it's called with a list with single item",
            )
        except Exception as e:
            self.fail(
                f"Function threw an error when it was called with a list with single item:\n{e}"
            )
    ...
    def test3_test_values(self):
        ...
        test_cases = (
            [("apple", 5), ("orange", 5)],
            [("apple", 4), ("orange", 5), ("banana", 6)],
            [("mouse", 2), ("hamster", 8), ("gerbil", 6)],
            [("car", 24), ("moped", 40), ("motorcycle", 10), ("truck", 5)],
            [
                ("rose", 100),
                ("daffodil", 90),
                ("sunflower", 80),
