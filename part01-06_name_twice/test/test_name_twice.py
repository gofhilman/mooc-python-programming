...
class NameTwiceTest(unittest.TestCase):
    ...
    def test_print_input_1(self):
        with patch("builtins.input", return_value="Paul"):
            ...
            self.assertTrue(
                splitted[0].rstrip() == "Paul" and splitted[1].rstrip() == "Paul",
                f"Your program did not work properply with input: Paul. Output was\n{output}\nExpected output is\nPekka\nPekka",
            )
    ...
    def test_print_input_2(self):
        with patch("builtins.input", return_value="Ada"):
            ...
            splitted = output.split("\n")
            self.assertTrue(
                splitted[0].rstrip() == "Ada" and splitted[1].rstrip() == "Ada",
                f"Your program did not work properply with input: Ada. Output was\n{output}\nExpected output is\nAda\nAda",
            )
