...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as list_of_stars(numbers: list)",
            )
        try:
            from src.list_of_stars import list_of_stars
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            val == None,
            f"Calling {function} should not return anything, now it returns value which type is {type(val)}",
        )
    ...
    def test_3_numbers_0(self):
        ...
        with patch(
            ...
            self.assertTrue(
                len(output) > 0,
                f"Your function does not print out anything when it is called as follows:\nlist_of_stars({test_case})",
            )
            self.assertEqual(
                c_rows,
                r_rows,
                f"The amount of the rows printed out is incorrect with the test input {test_case}. Your function printed out {r_rows} rows, correct amount is {c_rows}. The print out was:\n{output}\nit was expected to be:\n{correct}\nbe careful that you do not print out empty extra rows!",
            )
            self.assertEqual(
                correct,
                output,
