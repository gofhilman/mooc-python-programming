...
"""
    ...
    def test_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f'Your code should contain function named as squared, which can be called as follows:\nsquared("ab", 2)',
            )
        ...
        for word, size in [
            ...
            with patch("builtins.input", side_effect=["2"] * 100):
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f'Make sure that following function call works\nsquared("{word}", {size})',
                    )
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f'Function call squared("{word}", {size}) does not print out anything',
                )
                acual = "\n".join(output)
                self.assertEqual(
                    len(rows),
                    len(output),
                    f'Function call squared("{word}", {size}) should print out {size} rows, now it printed out {len(output)} rows, print out was\n{acual}',
                )
                ...
                for i in range(size):
                    self.assertEqual(
                        rows[i],
                        output[i].strip(),
                        f'Row {i}, which function call squared("{word}", {size}) prints out, should be {rows[i]}, now it is\n{output[i]}\nwhole print out of the function was\n{acual}',
                    )
