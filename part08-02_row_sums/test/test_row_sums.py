...
"""
    ...
    def test2_return_type(self):
        try:
            ...
            self.assertTrue(
                val == None,
                f"Function row_sums should not return a value,"
                + f" \nnow it returns a value {val} which is of type {taip}\n when it's called with arguments \n[[1,1],[2,2]]",
            )
        except Exception as e:
            self.fail(
                f"Function threw an error when called with arguments [[1,1],[2,2]]:\n{e}"
            )
        ...
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    test_case,
                    corr,
                    f"After executing the function, the matrix should be\n{corr}\nbut it is \n{test_case}\nwhen arguments are\n{test_case_2}",
                )
