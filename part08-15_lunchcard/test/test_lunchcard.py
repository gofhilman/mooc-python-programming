...
class LunchCardTest(unittest.TestCase):
    ...
    def test_0a_main_ok(self):
        with open("src/lunchcard.py") as t:
            if "if __name__" in t.read():
                self.fail(
                    'Main program must NOT be included inside if __name__ == "__main__": block'
                )
    ...
    def test2_constructor(self):
        try:
            ...
            self.fail("Calling constructor LunchCard(100)" + f" threw an error: {e}\n")
        ...
        for test_case in test_cases:
            try:
                ...
                self.assertEqual(
                    corr,
                    val,
                    f"Method __str__ should return string\n{corr}\nwhen object was initialized as\n"
...
block
        ...
        self.assertTrue(
            len(output_all) > 0, f"Your program does not output anything!\n{mssage}"
        )
        ...
        self.assertTrue(
            len(correct) == len(output),
            f"Your program should output {len(correct)} lines, now it outputs {len(output)} lines. Output was\n{output_all}",
        )
        ...
        for i in range(0, len(correct)):
            ...
            self.assertTrue(
                t == o,
                f"The output of your program at line {i+1} is incorrect. It should be\n{o}\nNow it is\n{t}\nThe whole output is\n{output_all}",
            )
    ...
    def test12_main_program_2(self):
        ...
        with open(src_file) as f:
            for line in f:
                for k in forbidden:
                    if k in line:
                        self.fail(
                            f"You must use LunchCard class in the exercise, the following line is forbidden:\n{line}"
                        )
        ...
        for v in required:
            ...
            self.assertTrue(
                on,
                f"You must use LunchCard class in the exercise, the following line must be included:\n{v}",
            )
