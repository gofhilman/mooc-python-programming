...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False, f"Your code should contain function named as triangle"
            )
        try:
            from src.triangle import triangle
            ...
            self.assertTrue(
                False,
                f"Make sure, that function can be called as follows:\rtriangle(5)",
            )
    ...
    def test_2_triangle_ok(self):
        for y_height, y_character in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f"Calling triangle({y_height}) does not print out anything",
                )
                acual = "\n".join(output)
                self.assertEqual(
                    len(exp),
                    len(output),
                    f"Calling triangle({y_height}) should print out {len(exp)} rows, now it prints out {len(output)} rows, the print out was\n{acual}",
                )
                for i in range(len(exp)):
                    self.assertEqual(
                        exp[i],
                        output[i].strip(),
                        f"The row {i+1} printed out after calling triangle({y_height}) should be:\n{exp[i]}\nbut it is:\n{output[i]}\nWhole print out of the function was\n{output_all}",
                    )
            ...
            for line in f:
                ...
                if in_function:
                    if "  print" in line:
                        self.assertTrue(
                            False,
                            f"Function triangle should not use print commands for printing, so following row is not allowed in your code\n{line}",
                        )
