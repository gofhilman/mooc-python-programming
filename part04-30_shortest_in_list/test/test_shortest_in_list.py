...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as shortest(my_list: list)",
            )
        try:
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == str,
            "Function shortest does not return value of string type with parameter value ['Alan', 'Steve'].",
        )

    def test_3_shortest_is_found(self):
        test_cases = {
            ("Alan", "Susan", "Seymour", "Kim", "Susan"): ["Kim"],
        ...
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    correct,
                    test_result,
                    f"The result {test_result} does not match with the expected result {correct} when calling shortest{test_case2}",
                )
                self.assertEqual(
                    test_case,
                    test_case2,
