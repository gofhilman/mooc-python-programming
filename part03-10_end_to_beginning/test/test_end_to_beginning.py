        ...
        for test_case in values:
            with patch("builtins.input", side_effect=[test_case]):
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure that your program works with the input {test_case}",
                    )
                output = get_stdout()
                ...
                self.assertEqual(
                    len(output_list),
                    len(cor),
                    f"Your program is expected to print out {len(cor)} rows, now your program's print out is in {len(output_list)} rows.",
                )
                r = 1
                for l1, l2 in zip(cor, output_list):
                    self.assertTrue(
                        sanitize(l1) == sanitize(l2),
                        f"When the input is {test_case} print out in row {r+1} is incorrect, row should be\n{l1}\nbut it is\n{l2}",
                    )
