        ...
        for word in words:
            with patch("builtins.input", return_value=word):
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure that your program's works with the input {word}",
                    )
                ...
                self.assertTrue(
                    len(output) == 1,
                    "With the input {} instead of one row, your program's print out is in {} rows".format(
                        word, len(output)
                    ),
                )
                self.assertTrue(
                    outputs_equal(output[0], correct),
                    "Your program's print out\n{}\ndoes not match with the correct print out\n{} \nwith the input {}".format(
                        output[0], correct, word
                    ),
                )
