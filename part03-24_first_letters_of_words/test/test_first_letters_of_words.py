...
class FirstLettersOfWordsTest(unittest.TestCase):
    ...
    def test_short_sentences(self):
        words = [
            ...
            "Here is one more sentence for test",
        ]
        for testcase in words:
            with patch("builtins.input", return_value=testcase):
                try:
                    ...
                    self.assertFalse(
                        True,
                        f"Make sure that the execution of the program works with the input\n{testcase}",
                    )
                ...
                self.assertTrue(
                    ...
                    "Instead of {} rows, your program prints out {} rows with the input {}\n{}".format(
                        len_correct, len(output), testcase, output_all
                    ),
                )
