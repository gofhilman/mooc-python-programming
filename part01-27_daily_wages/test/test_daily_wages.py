...
class DailyWagesTest(unittest.TestCase):
    ...
    def test_other_days_1(self):
        ...
        with patch("builtins.input", side_effect=ilist):
            ...
            self.assertTrue(
                len(output_list) == 1,
                "Instead of one row, your program's output is now in {} rows".format(
                    len(output_list)
                ),
            )
            self.assertTrue(
                output.find(str(hours * perhour)) > -1,
                "With input {} correct wage {} is not found in output {}".format(
                    test_input, (hours * perhour), output
                ),
            )
            ...
            self.assertEqual(
                output,
                correct,
                "Print out is in incorrect format, correct print out is\n{}\nbut now print out is\n{}".format(
                    correct, output
                ),
            )
