...
"""
    ...
    def test_1_function_copy_and_add_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should contain function named as copy_and_add(sudoku: list, row_no: int, column_no: int, number:int)",
            )
        ...
        try:
            ...
            self.assertTrue(
                False,
                f"Make sure, that function can be called as follows\n{p(s)}\ncopy_and_add(s, 0, 1, 3)",
            )
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertFalse(
            res == None,
            f"Function copy_and_add should return two-dimensional list of integer values. Now function does not return anything",
        )
        self.assertTrue(
            type(res) is list,
            f"Function copy_and_add should return two-dimensional list of integer values. The return value of the function is now\n{res}",
        )
        try:
            self.assertTrue(
                type(res[0]) is list,
    ...
    def test_3_function_works_1(self):
        ...
        for r, s, number in [(1, 1, 5), (0, 0, 1), (3, 4, 7), (2, 5, 8), (3, 5, 2)]:
            ...
            try:
                ...
                self.assertTrue(
                    False,
                    f"Make sure, that the following function call works\n{p(sudoku)}",
                )
            ...
            for rno in range(9):
                ...
                self.assertEqual(
                    sudoku[rno],
                    o,
                    f"The row {rno} in matrix returned after calling\n{p(sudoku)}\nres = copy_and_add(s, {r}, {s}, {number})\n {rno} should still be:\n{o}:\nbut it has been changed to:\n{res[rno]}",
                )
    ...
    def test_4_function_works_2(self):
        ...
        for r, s, number in [(1, 1, 5), (0, 0, 1), (3, 4, 7), (2, 5, 8), (3, 5, 2)]:
            ...
            try:
                ...
                self.assertTrue(
                    False,
                    f"Make sure, that following function call works properly:\n{p(sudoku)}",
                )
