    ...
    def test_function_exists(self):
        with patch("builtins.input", side_effect=["2"] * 10):
            try:
                ...
                self.assertTrue(
                    False,
                    f'Your code should contain function named as first_character, which can be called as follows:\nfirst_character("testing")',
                )
    ...
    def test_correct_characters(self):
        for word in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertTrue(
                    len(output) > 0,
                    f'Calling function first_character("{word}") does not print out anything',
                )
                self.assertEqual(
                    1,
                    len(output),
                    f'Function call first_character("{word}") should print out only one character, now it printed out {len(output)} characters, print out was\n{output}',
                )
                self.assertEqual(
                    word[0],
                    output,
                    f'Function call first_character("{word}") should print out only one character, {word[0]}, now it printed out\n{output}',
                )
