    ...
    def test_1_exit_only(self):
        ...
        with patch("builtins.input", side_effect=input_data):
            try:
                ...
                self.fail(
                    f"Ensure that program can be executed with input:\n{f(input_data)}"
                )
            output = get_stdout()
            correct = "Bye"
...
"""
            ...
            self.assertTrue(
                len(output.split("\n")) == 2,
                f"Program should output two lines with input\n{f(input_data)} now the output is \n{output}",
            )
            self.assertTrue(
                correct in output,
                f"Program should output {correct} before stopping, now the output is \n{output}",
            )
    ...
    def test_4b_load_again_and_output(self):
        ...
        with patch("builtins.input", side_effect=input_data):
            ...
            self.assertTrue(
                correct in output,
                f"When the program is first executed with input data\n{f(input_data1)}\nand again with input data:\n{f(input_data)}\not should output \n{correct}\nnow it outputs \n{output}",
            )
