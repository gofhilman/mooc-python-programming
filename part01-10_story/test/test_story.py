...
class StoryTest(unittest.TestCase):
    ...
    def test_print_1(self):
        ...
        with patch("builtins.input", side_effect=plist):
            ...
            self.assertTrue(
                len(output) > 0,
                "Your program does not print anything with input\nJeremy\n1340",
            )
            ...
            for piece in plist:
                self.assertTrue(
                    ...
                    "Input "
                    + piece
                    + " was not included in output, when input was\nJeremy\n1340",
                )

            story = (
                plist[0] + " is a valiant knight, born in the year " + plist[1] + "."
            )
            story += (
                " One morning "
                + plist[0]
                + " woke up to an awful racket: a dragon was approaching the village. "
            )
    ...
    def test_print_3(self):
        ...
        with patch("builtins.input", side_effect=plist):
            ...
            for piece in plist:
                self.assertTrue(
                    output.find(piece) > -1,
                    "Input " + piece + " was not included in output",
                )
