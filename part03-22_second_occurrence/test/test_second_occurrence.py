        ...
        for testcase in words:
            with patch("builtins.input", side_effect=testcase.split(",")):
                ...
                try:
                    ...
                    self.assertFalse(
                        True, "Make sure that your program works with the input " + word
                    )
                ...
                self.assertFalse(
                    len(output_all) == 0,
                    "Your program does not print out anything with the input "
                    + f(testcase),
                )
                self.assertTrue(
                    len(output) == 1,
                    "Instead of {} rows, your program prints out {} rows with the input {}\n{}".format(
                        1, len(output), testcase, output_all
                    ),
                )

                self.assertTrue(
                    str(correct) in output_all,
                    "Correct answer {} is not found from the print out when the input is\n{}\nThe program printed out:\n{}".format(
                        correct, f(testcase), output_all
                    ),
                )

                correct_str = (
                    "The second occurrence of the substring is at index "
                    + str(correct)
                    + "."
                )

                self.assertTrue(
                    outputs_equal(output_all, correct_str),
                    "Your program's print out\n{}\ndoes not match with the correct print out\n{} \nwhen the input is ({})".format(
                        output_all, correct_str, testcase
                    ),
                )
        ...
        for testcase in words:
            with patch("builtins.input", side_effect=testcase.split(",")):
                ...
                try:
                    ...
                    self.assertFalse(
                        True,
                        "Make sure that your program works with the input "
                        + f(testcase),
                    )
                output_all = get_stdout()
                output = [
