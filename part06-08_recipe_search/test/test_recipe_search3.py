...
class RecipeSearchPart3Test(unittest.TestCase):
    ...
    def test_7_search_by_ingredient_exists(self):
        with patch("builtins.input", side_effect=["recipes1.txt"] * 10):
            ...
            try:
                ...
                self.fail(
                    'Function call search_by_ingredient("test/recipes1.txt", "milk") throws an error:\n'
                    + ioe
                )
    ...
    def test_8_search_by_ingredient_return_type(self):
        with patch("builtins.input", side_effect=["recipes1.txt"]):
            ...
            self.assertTrue(
                type(answer) == list,
                f"Function {function3} should return a list, now it returns value {answer} which is of type {taip}.",
            )
...
Cake pops, preparation time 60 min""".split(
            ...
            self.assertEqual(
                len(exp),
                len(answer),
                f"Function returns a wrong number of recipes when called with \n{code}",
            )
            for r in answer:
                self.assertTrue(
                    r in exp,
                    f"Returned wrong recipe {r} when called with {code}\nThe right return value is {exp}",
                )
            for r in exp:
                self.assertTrue(
                    r in answer,
                    f"When called with {code}\nreturned recipes should contain {r}\nNow function returns {answer}",
                )
