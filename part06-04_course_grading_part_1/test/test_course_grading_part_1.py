...
class CourseGradingPart1Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        with patch(
            "builtins.input",
            side_effect=["test/students1.csv", "test/exercises1.csv", "third"],
        ):
    ...
    def test_1_works_with_files_1(self):
        ...
        with patch(
            "builtins.input",
            side_effect=w(words) + [AssertionError("Too many inputs.")],
        ):
            try:
                ...
                self.assertTrue(
                    False, f"Ensure that your progaram can be executed with\n{f(words)}"
                )
...
"""
            ...
            self.assertTrue(
                len(output_all) > 0,
                f"Your program doesn't output anything with input\n{f(words)}\n{mssage}",
            )
            output = [line.strip() for line in output_all.split("\n") if len(line) > 0]
            self.assertEqual(
                len(expRows),
                len(output),
                f"Instead of {len(expRows)} rows, your program outputs {len(output)} rows:\n{output_all}\nwith input:\n{f(words)}\nOutput should be:\n{exp}",
            )
            for i in range(len(expRows)):
                line = output[i]
                self.assertTrue(
                    line.strip() in exp,
                    f"Your program does not work with input\n{f(words)}\nrow {line} is not what was expected\nTThe output should contain the following lines:\n{exp}\nWhole output is:\n{output_all}",
                )
