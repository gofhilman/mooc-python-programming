...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as sum_of_positives(my_list: list)",
            )
        try:
            from src.sum_of_positives import sum_of_positives
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == int,
            f"Calling {function} does not return value of integer type with parameter values sum_of_positives([1,2])",
        )
        ...
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    correct,
                    test_result,
                    f"The result {test_result} does not match with the correct answer {correct} when calling sum_of_positives({test_case2})",
                )
                self.assertEqual(
                    test_case,
                    test_case2,
