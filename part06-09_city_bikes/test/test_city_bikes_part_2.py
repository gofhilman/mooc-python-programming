...
class CityBikesPart2Test(unittest.TestCase):
    ...
    def test_1_greatest_distance_exicsts(self):
        try:
            ...
            self.fail(
                "Your code should contain function greatest_distance(stations: dict)"
            )
...
greatest_distance(stations)"""
        ...
        self.assertTrue(
            type(val) == tuple,
            f"Function {function2} should return a tuple, now it returns {val} which is of type {taip}.",
        )
        taip = str(type(val[0])).replace("<class '", "").replace("'>", "")
        self.assertTrue(
            type(val[0]) == str,
            f"The first item in a tuple returned by {function2} should be a string, now the type is {taip}\nReturn value was {val}",
        )
        taip = str(type(val[1])).replace("<class '", "").replace("'>", "")
        self.assertTrue(
            type(val[1]) == str,
            f"The second item in a tuple returned by {function2} should be a string, now the type is {taip}\nReturn value was {val}",
        )
        taip = str(type(val[2])).replace("<class '", "").replace("'>", "")
        self.assertTrue(
            type(val[2]) == float or type(val[2]) == int,
            f"The third item in a tuple returned by {function2} should be a float, now the type is {taip}\nReturn value was {val}",
        )
    ...
    def test_3_greatest_distance_works(self):
        for filename, answer in [
            ...
            (
                "stations3.csv",
                ("Puotinkylan kartano", "Friisilanaukio", 21.971314423058754),
            ),
            ("stations4.csv", ("Kaivopuisto", "Linnuntie", 11.569340603194116)),
            (
                "stations5.csv",
                ("Puotinkylan kartano", "Etuniementie", 21.8490934564622),
            ),
            ...
            self.assertTrue(
                (a1 == ma1 and a2 == ma2) or (a2 == ma1 and a1 == ma2),
                f"Answer is wrong when executed code is \n{code}\nLongest distance is between stations {ma1} and {ma2}\nYour function returns {pal}",
            )
            self.assertTrue(
                close(et, met),
