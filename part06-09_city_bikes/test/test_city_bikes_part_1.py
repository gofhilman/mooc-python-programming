...
class CityBikesPart1Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        with patch(
            "builtins.input", side_effect=[AssertionError("Input was not expected")]
        ):
...
"""
    ...
    def test_1_get_station_data_exists(self):
        try:
            ...
            self.fail(
                "Your code should contain function get_station_data(filename: str)"
            )
        ...
        try:
            ...
            self.assertTrue(
                False,
                'Function call get_station_data("stations1.csv") throws an error ' + e,
            )
    ...
    def test_2_get_station_data_return_type(self):
        ...
        self.assertTrue(
            type(val) == dict,
            f"Function {function1} should return a dictionary, now it returns a value {val} which is of type {taip}.",
        )
    ...
    def test_3_get_station_data_works(self):
        exp = {
            ...
            "Kapteeninpuistikko": (24.944927399779715, 60.15818919997167),
            "Viiskulma": (24.941775800312996, 60.16098589997938),
            "Sepankatu": (24.93628529982675, 60.157948300373846),
            "Hietalahdentori": (24.92970990039163, 60.162225100108344),
            "Designmuseo": (24.94595999955436, 60.163103199952786),
            "Vanha kirkkopuisto": (24.939149900447603, 60.165288299815245),
            "Erottajan aukio": (24.944134928883898, 60.16691166693994),
        }
        get_station_data = load(exercise, function1, "en")
        val = get_station_data("stations1.csv")
        ...
        for a, k in exp.items():
            self.assertTrue(
                a in val,
                f"Dictionary returned by {code} should contain key {a}. Now function returns\n{val}",
            )
            taip = str(type(val[a])).replace("<class '", "").replace("'>", "")
            self.assertEqual(
                type(val[a]),
                tuple,
                f"Key {a} returned by {code} should be a tuple\nnow it is {val[a]}\nand the type is {taip}",
            )
            taip = str(type(val[a][0])).replace("<class '", "").replace("'>", "")
            self.assertEqual(
                type(val[a][0]),
                float,
                f"The key {a} returned by {code} should contain coordinates of type float.\nNow the value is {val[a][0]} and the type is {taip}",
            )
            ...
            self.assertEqual(
                k,
                val[a],
                f"The value of the key {a} returned by {code} should be\n{k}\nnow it was\n{val[a]}",
            )
            self.assertEqual(
                sorted(list(val.keys())),
                sorted(list(exp.keys())),
                f"The dictionary returned by {code} should contain the following keys:\n{sorted(list(exp.keys()))}\nnow the returned keys are:\n{sorted(list(val.keys()))}",
            )
    ...
    def test_4_distance_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Code should contain function distance(stations: dict, station1: str, station2: str)",
            )
...
distance(stations, "Kaivopuisto", "Laivasillankatu")"""
        ...
        self.assertTrue(
            type(val) == float,
            f"Function {function2} should return a float type value, now it returns  {val} which is of type {taip}.",
        )
    ...
    def test_6_distance_works(self):
        for start, end, dist in [
            ...
            self.assertTrue(
                close(val, dist),
                f"The answer {val} returned by function is wrong, it should be {dist} when called with\n{code}",
            )
