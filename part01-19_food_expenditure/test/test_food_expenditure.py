...
class FoodExpenditureTest(unittest.TestCase):
    ...
    def test_0(self):
        with patch(
            ...
            self.assertFalse(
                prompt.call_count < 2,
                "The program is expected to ask input three times.",
            )
            expected = "Daily: 4.5 euros"
            self.assertTrue(
                sanitize(expected) in sanitize(output),
                f"With inputs 4, 2.5 and 21.5 output is expected to contain row:\n{expected}\nyour program's output was:\n{output}",
            )
    ...
    def test_2_additional_tests(self):
        ...
        for a, b, c, d, w in testset:
            with patch(
                "builtins.input",
                side_effect=[a, b, c, AssertionError("Input is asked too many times.")],
            ) as prompt:
                reload_module(self.module)
                output = get_stdout()
                inputs = f"{a}, {b}, and {c}"
                self.assertFalse(
                    prompt.call_count < 3,
                    "The program should ask for input three times.",
                )
                expected = f"Daily: {d} euros"
                self.assertTrue(
                    sanitize(expected) in sanitize(output),
                    f"with inputs {inputs} output is expected to contain row:\n{expected}\nyour program's output was:\n{output}",
                )
                expected = f"Weekly: {w} euros"
                self.assertTrue(
                    sanitize(expected) in sanitize(output),
                    f"With {inputs} output is expected to contain row:\n{expected}\nyour program's output was:\n{output}",
                )
