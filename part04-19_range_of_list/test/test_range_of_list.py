...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should contain function named as range_of_list(list: list)",
            )
        try:
            from src.range_of_list import range_of_list
    ...
    def test_2_functionality_ok(self):
        for list in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertFalse(
                    res == None,
                    f"Calling range_of_list({f(list)}) should return {expected} now it does not return anything. Make sure that you use return command in any cases in your function!",
                )
                self.assertEqual(
                    res,
                    expected,
                    f"Calling range_of_list({f(list)}) should return {expected} now it returns {res}",
                )
                self.assertFalse(
                    len(output_all) > 0,
                    f"Calling range_of_list({f(list)}) should not print out anything, but it prints out\n{output_all}\nremove print commands inside function",
                )
