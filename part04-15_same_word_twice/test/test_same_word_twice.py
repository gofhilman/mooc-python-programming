    ...
    def test_1_program_stops(self):
        ...
        with patch(
            ...
            try:
                ...
                self.assertTrue(
                    False,
                    f"Make sure that the execution of the program stops with the input\n{f(words)}",
                )
...
block
                ...
                self.assertEqual(
                    1,
                    len(output),
                    f"Instead of one row, your programs print out is now in {len(output)} rows:\n{output_all}\nwith the input:\n{f(words)}",
                )
                correct = "You typed in " + str(len(words) - 1) + " different words"
                self.assertTrue(
                    output[0].rstrip() == correct,
                    f"The print out of your program is incorrect, it should be\n{correct}\nrow is\n{output[0]}\nwhen the input is:\n{f(words)}",
                )
