...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False, f"Your code should contain function named as length(items: list)"
            )
        try:
            from src.length_of_list import length
            ...
            self.assertTrue(
                False,
                f"Make sure, that function can be called as follows length([1, 2, 3])",
            )
    ...
    def test_2_functionality_ok(self):
        for list in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertFalse(
                    res == None,
                    f"Calling length({f(list)}) should return {expected} now it does not return anything. Make sure that you use return command in any cases in your function!",
                )
                self.assertEqual(
                    res,
                    expected,
                    f"Calling length({f(list)}) should return {expected} now it returns {res}",
                )
                self.assertFalse(
                    len(output_all) > 0,
                    f"Calling length({f(list)}) should not print out anything, but it prints out\n{output_all}\nremove print commands inside function",
                )
