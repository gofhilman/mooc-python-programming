...
class NextLeapYearTest(unittest.TestCase):
    ...
    def test_2019(self):
        with patch(
            "builtins.input",
            side_effect=["2019", AssertionError("Input is asked too many times.")],
        ) as prompt:
            ...
            self.assertFalse(
                len(output) == 0,
                f"With the input 2019 your program should print out\n{expected}\nnow your program did not print out anything",
            )
            self.assertEqual(
                sanitize(expected),
                sanitize(output),
    ...
    def test_2020(self):
        with patch(
            "builtins.input",
            side_effect=["2020", AssertionError("Input is asked too many times.")],
        ) as prompt:
            reload_module(self.module)
            output = get_stdout()
            ...
            self.assertFalse(
                len(output) == 0,
                f"Your program did not print out anything with the input 2024",
            )
    ...
    def test_1896(self):
        with patch(
            "builtins.input",
            side_effect=["1896", AssertionError("Input is asked too many times.")],
        ) as prompt:
            reload_module(self.module)
            output = get_stdout()
            ...
            self.assertFalse(
                len(output) == 0,
                f"Your program did not print out anything with the input 1896",
            )
