    ...
    def test_function_exists(self):
        with patch("builtins.input", side_effect=["2"] * 10):
            try:
                ...
                self.assertTrue(
                    False,
                    f"Your code should contain function named as mean, which can be called as follows:\nmean(1,2,3)",
                )
    ...
    def test_calculation_is_correct(self):
        for l1, l2, l3 in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f"Calling function mean{inpt} does not print out anything",
                )
                acual = "\n".join(output)
                self.assertEqual(
                    1,
                    len(output),
                    f"Function call mean({inpt}) should print out only 1 row, now it printed out {len(output)} rows, print out was\n{acual}",
                )
                expctd = (l1 + l2 + l3) / 3
                try:
                    ...
                    self.assertFalse(
                        True,
                        f"Function call mean({inpt}) should print out {expctd}, print out of the function was\n{acual}\nis not convertible to a floating point number!",
                    )
                ...
                self.assertAlmostEqual(
                    ...
                    was,
                    2,
                    f"Function call mean({inpt}) should print out {expctd}, now it printed out\n{was}",
                )
