...
def par(t1: tuple, t2: tuple, t3: tuple):
    ...
    for t in (t1, t2, t3):
        s += (
            "{" + ",".join([f'"result{x}": {t[x-1]}' for x in range(1, 4)]) + "}" + ", "
        )
    return s[:-2] + ")"
...
class SmallestAverageTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        with patch(
            "builtins.input", side_effect=[AssertionError("Input wasn't expected")]
        ):
...
"""
    ...
    def test1_function_exists(self):
        try:
            ...
            self.fail(
                "Your program must contain a function smallest_average(p1: dict, p2: dict, p3: dict)"
            )
    ...
    def test2_return_type(self):
        try:
            ...
            self.assertTrue(
                type(val) == dict,
                f"Function smallest_average should return a value of type dictg,"
                + f" now it returns {val} which is of type {taip}\nwhen called with parameters {par((1,1,1),(2,2,2),(3,3,3))}",
            )
        except:
            self.fail(
                f"Function threw an error when called with parameter values {par((1,1,1),(2,2,2),(3,3,3))}"
            )
    ...
    def test3_test_values(self):
        ...
        for test_case in test_cases:
            with patch(
                ...
                corr = min(
                    (t1, t2, t3),
                    key=lambda x: ((x["result1"] + x["result2"] + x["result3"]) / 3),
                )

                self.assertEqual(
                    val,
                    corr,
                    f"Function should return \n{corr}\nbut it returns \n{val}\nwhen parameters are\n{par(test_case[0], test_case[1], test_case[2])}",
                )
