...
class SpruceTest(unittest.TestCase):
    ...
    def test_function_exists(self):
        ...
        try:
            ...
            self.assertTrue(
                False, f"Make sure, that function can be called as follows:\rspruce(3)"
            )
    ...
    def test_spruce_ok(self):
        for height in [3, 4, 5, 7, 10, 14, 18, 20, 25, 50]:
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f"Calling spruce({height}) does not print out anything",
                )
                acual = "\n".join(output)
                self.assertEqual(
                    "a spruce!",
                    output[0].rstrip(),
                    f"First row in the print out after calling spruce({height}) should be:\na spruce!\nbut it is:\n{output[0]}",
                )
                self.assertEqual(
                    len(exp),
                    len(output),
                    f"Calling spruce({height}) should print out {len(exp)} rows, now it prints out {len(output)} rows, the print out was\n{acual}",
                )
                acual_spruce = "\n".join(exp)
