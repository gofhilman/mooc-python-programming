...
"""
    ...
    def test2_constructor(self):
        try:
            ...
            self.assertTrue(
                False,
                'Calling the constructor as Person("Peter Python")'
                + f" threw an error: {e}",
            )
        ...
        for test_case in test_cases:
            try:
                ...
                self.assertEqual(
                    val,
                    corr,
                    f"Method return_first_name should return {corr}, when person was initalized asa\n"
                    + f"Person('{test_case}')\nNow method returns\n{val}",
                )

            except Exception as e:
                self.fail(
                    f"There was an error when calling method return_first_name:\n{e}"
        ...
        for test_case in test_cases:
            try:
                ...
                self.assertEqual(
                    val,
                    corr,
                    f"Mtehod return_last_name should return {corr}, when the object was initalized as\n"
                    + f"Person('{test_case}')\nNow the method returns\n{val}",
                )

            except Exception as e:
                self.fail(
                    f"There was an error when calling method return_last_name:\n{e}"
                    + f"The object was initalized as Person{(test_case)}"
                )
    ...
    def test5_test_attributes(self):
        ...
        self.assertTrue(
            len(att_list) == 1,
            f"Class Person should have only one attribute, now there are {len(att_list)}\n"
