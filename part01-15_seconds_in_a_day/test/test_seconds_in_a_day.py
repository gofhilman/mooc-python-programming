...
class SecondsInADayTest(unittest.TestCase):
    ...
    def test_1_seconds_in_one_day(self):
        with patch("builtins.input", return_value="1"):
            ...
            self.assertTrue(
                "86400" in result.split(),
                "1 day is 86400 seconds. Your program's output was: "
                + parse_result(result),
            )
    ...
    def test_2_seconds_in_seven_days(self):
        with patch("builtins.input", return_value="7"):
            ...
            result = get_stdout()
            self.assertTrue(
                "604800" in result.split(),
                "7 days is 604800 seconds. Your program's output was: "
                + parse_result(result),
            )
    ...
    def test_3_additional_tests(self):
        ...
        for days, seconds in testset:
            with patch("builtins.input", return_value=days):
                reload_module(self.module)
                self.assertTrue(
                    seconds in get_stdout().split(),
                    "Output is incorrect with input "
                    + days
                    + ". "
                    + days
                    + " days is "
