...
class AbsolutevalueTest(unittest.TestCase):
    ...
    def test_1_miinus_8(self):
        with patch("builtins.input", return_value="-8"):
            ...
            output = get_stdout()
            self.assertTrue(
                "The absolute value of this number is 8" in output,
                "with input -8 your program should print out\nThe absolute value of this number is 8\nyour program printed out\n"
                + output,
            )
    ...
    def test_2_plus_2(self):
        with patch("builtins.input", return_value="2"):
            ...
            output = get_stdout()
            self.assertTrue(
                "The absolute value of this number is 2" in output,
                "with input 2 your program should print out\nThe absolute value of this number is 2\nyour program printed out\n"
                + output,
            )
    ...
    def test_3_additional_tests(self):
        ...
        for number in testset:
            with patch("builtins.input", return_value=number):
                ...
                if int(number) >= 0:
                    self.assertTrue(
                        f"-{number}" not in get_stdout(),
                        "Your program works incorrectly with input "
                        + number
                        + ". Answer should be "
                        + result,
                    )
                self.assertTrue(
                    result in get_stdout(),
                    "Your program works incorrectly with input "
                    + number
                    + ". Answer should be "
                    + result,
