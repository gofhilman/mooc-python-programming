...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as times_ten(start_index: int, end_index: int)",
            )
        try:
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == dict,
            f"Function {function} should return value which is dict-type, now it returns value {val} which is {taip}-type.",
        )
        ...
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    len(correct),
                    len(value),
                    f"The returned dictionary should contain {len(correct)} items, but it contains {len(value)} items: \n{value} when the parameters are {test_case}",
                )
                self.assertEqual(
                    value,
                    correct,
