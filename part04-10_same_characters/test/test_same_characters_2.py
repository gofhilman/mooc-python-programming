        ...
        for mj, a, b in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f'Make sure that calling function same_chars("{mj}", {a}, {b}) works',
                    )
                ...
                self.assertFalse(
                    ret == None,
                    f'Calling same_chars("{mj}", {a}, {b}) should return {expected} now it does not return anything. Make sure that you use return command in all cases in your function!',
                )
                self.assertEqual(
                    ret,
                    expected,
                    f'Calling same_chars("{mj}", {a}, {b}) should return {expected} now it returns {ret}',
                )
                self.assertFalse(
                    len(output_all) > 0,
                    f'Calling same_chars("{mj}", {a}, {b}) should not print out anything, but it prints out\n{output_all}\nremove print commands inside function',
                )
