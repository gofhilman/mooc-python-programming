    ...
    def test_1_negatives(self):
        ...
        for value in values:
            with patch("builtins.input", return_value=value):
                ...
                self.assertTrue(
                    len(output) == 1,
                    "Instead of one row, your program's output is now in {} rows: {} when input is {}".format(
                        len(output), output, value
                    ),
                )
                self.assertTrue(
                    output[0].find("impossible!") > -1,
                    "From output\n{}\nmention 'impossible!' is not found when input is {}".format(
                        output[0], value
                    ),
                )
    ...
    def test_3_fails(self):
        ...
        for value in values:
            with patch("builtins.input", return_value=value):
                ...
                self.assertTrue(
                    output[0].find("fail") > -1,
                    "From output\n{}\nmention 'fail' is not found when input is {}".format(
                        output[0], value
                    ),
                )
