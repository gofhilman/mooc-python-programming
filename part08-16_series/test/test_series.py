...
"""
    ...
    def test2_constructor(self):
        try:
            ...
            self.fail(
                'Calling constructor as Series("Dexter", 8, ["Crime", "Drama", "Mystery", "Thriller"])'
    ...
    def test3_test_str(self):
        ...
        try:
            ...
            corr = (
                f"{test_case[0]} ({test_case[1]} seasons)\ngenres: {genres}\nno ratings"
            )
            val = str(series)
            ...
            self.assertEqual(
                sanitize(corr),
                sanitize(val),
                f"Method __str__ should return a string\n{corr}\nwhen object was created as\n"
                + f"{code}\nNow method returns\n{val}",
            )

        except Exception as e:
            self.fail(
                f"Calling method __str__ threw an error: {e}\nwhen object was created as\n{code}"
            )
...
"""
    ...
    def test6_function_minimum_grade_olemassa(self):
        try:
            ...
            self.fail(
                "Your program should have a function called minimum_grade(grade: float, series: list)"
            )
...
"""
        ...
        self.assertTrue(
            type(answer) == list,
            "Function minimum_grade(rating: float, series: list) should return a list",
        )

        expected = 1
        self.assertTrue(
            len(answer) == expected,
            f"When this code is executed\n{code}\nthe length of the list returned should be {expected}, however, it was {len(answer)}",
        )
        self.assertTrue(
            answer[0].title == "Dexter",
...
"""
        ...
        self.assertTrue(
            type(answer) == list,
            "Function minimum_grade(grade: float, series: list) should return a list",
        )
        ...
        ehto = (answer[0].title == "Dexter" and answer[1].title == "South Park") or (
            answer[1].title == "Dexter" and answer[0].title == "South Park"
        )
        self.assertTrue(
            ehto,
            f"When this code is executed code\n{code}\nthe list should include Dexter and South park, now the list was {answer[0].title} ja {answer[1].title}",
        )
    ...
    def test8_function_includes_genre_olemassa(self):
        try:
            ...
            self.fail(
                "Your program should include a function  includes_genre(genre: str, series: list)"
            )
...
"""
        ...
        self.assertTrue(
            type(answer) == list,
            "Function includes_genre(genre: str, series: list) should return a list",
        )
...
"""
        ...
        ehto = (answer[0].title == "Friends" and answer[1].title == "South Park") or (
            answer[1].title == "Friends" and answer[0].title == "South Park"
        )
        self.assertTrue(
            ehto,
