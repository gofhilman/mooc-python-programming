...
"""
    ...
    def test_2_return_type(self):
        ...
        try:
            ...
            self.assertTrue(
                False, f"Ensure that function can be called:\nread_fruits()"
            )
        taip = str(type(val)).replace("<class '", "").replace("'>", "")
        self.assertTrue(
            type(val) == dict,
            f"Function {function} should return a dictionary, now it returns value {val} which is of type {taip}.",
        )
