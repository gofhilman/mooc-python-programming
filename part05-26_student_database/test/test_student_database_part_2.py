...
class StudentDatabasePart2Test(unittest.TestCase):
    ...
    def test_part2_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should conorn function named as {function3}(students: dict, name: str, completion: tuple)",
            )
...
print_student(students, "Peter")"""
 ...
 average grade 4.0"""
        ...
        self.assertFalse(
            len(output_all) == 0,
            f"Your program is expected to print out {len(exp1)} rows when executing the following code:\n{code}\nNow it does not print out anything",
        )
        self.assertEqual(
            len(output),
            len(exp1),
        ...
        for i in range(len(exp1)):
            if exp1[i] == exp2[i]:
                self.assertTrue(
                    output[i].rstrip() == exp1[i] or output[i].rstrip() == exp2[i],
                    f"When executing the following code:\n{code}\nThe row number {i+1} should be:\n{exp1[i]}\nbut it is\n{output[i]}",
                )
            else:
                self.assertTrue(
