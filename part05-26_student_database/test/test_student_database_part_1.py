...
"""
    ...
    def test_part1_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should contain function named as {function1}(students: dict, name: str)",
            )
        try:
...
add_student(students, "Peter")"""
            self.assertTrue(
                False,
                f"Make sure, that the function can be called as in the following code: \n{code}",
            )
    ...
    def test_part1_2_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should contain function named as {function2}(students: dict, name: str)",
            )
        try:
...
print_student(students, "Peter")"""
            ...
            self.assertTrue(
                False,
                f"Make sure, that the function can be called as in the following code: {code}",
            )
...
print_student(students, "Peter")"""
 ...
 no completed courses"""
            ...
            self.assertTrue(
                False, f"Make sure, the execution of the following code works:\n{code}"
            )

        self.assertFalse(
            len(output_all) == 0,
            f"Your program is expected to print out {len(exp)} rows when executing the following code:\n{code}\nNow it does not print out anything",
        )
        self.assertEqual(
            len(output),
            len(exp),
            f"Your program is ecpected to print out {len(exp)} rows when executing the follwing code:\n{code}\nNow it prints out {len(output)} rows:\n{output_all}",
        )
        for i in range(len(exp)):
            self.assertEqual(
                output[i].rstrip(),
                exp[i],
                f"When executing the following code:\n{code}\nThe row number {i+1} should be:\n{exp[i]}\nbut it is:\n{output[i]}",
            )
