...
testset = [
    ...
    + ["end"],
    "The focus of the course is on programming and you will learn how to write programs and understand how they work For example the basics of algorithms control structures subprograms object-oriented programming are covered The grading is based on weekly exercises and an online exam".split(
        " "
    )
    + ["end"],
]
    ...
    def test_part1b(self):
        for *start, end in testset:
            with patch(
                "builtins.input",
                side_effect=start
                + [end, AssertionError("Input is asked too many times.")],
            ) as prompt:
                reload_module(self.module)
                output = get_stdout()
                ...
                self.assertTrue(
                    len(output) > 0,
                    f"Your program did not print out anything with the input\n{inpt}",
                )
                self.assertEqual(
                    expected.strip(),
                    output.strip(),
