...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as most_common_character(my_string: str)",
            )
        try:
            from src.most_common_character import most_common_character
            ...
            self.assertTrue(
                False,
                'Make sure, that function can be called as most_common_character("abc")',
            )
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == str,
            'Function most_common_character does not return value of string type when calling most_common_character("abc")',
        )
        ...
        for test_case in ["aaabb", "aabbbbc", "abcabca", "xyzxyzyyxyz", "composer"]:
            with patch(
                ...
                self.assertEqual(
                    correct,
                    test_result,
                    f"The result {test_result} does not match with the expected result {correct} when calling most_common_character({f(test_case)})",
                )

    def test_4_several_words(self):
        for test_case in [
            "aaaa bbb ccc ddddd bbb",
            "she sells seasell",
            "xyz xyz xyz zzzzorro",
        ]:
            with patch(
                "builtins.input",
                side_effect=[
