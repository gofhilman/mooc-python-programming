...
"""
    ...
    def test_1_function_exists(self):
        ...
        try:
            ...
            self.assertTrue(
                False,
                f'Make sure, that function can be called as follows:\rshape(5, "-", 3, "*")',
            )

    def test_2_shape_ok(self):
        for y_height, y_character, a_height, a_character in [
            (5, "X", 3, "*"),
            (3, "X", 1, "*"),
            (4, "x", 5, "X"),
            (4, "x", 0, "X"),
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f'Calling shape({y_height}, "{y_character}", {a_height}, "{a_character}") does not print out anything',
                )
                acual = "\n".join(output)
                self.assertEqual(
                    len(exp),
                    len(output),
                    f'Calling shape({y_height}, "{y_character}", {a_height}, "{a_character}") should print out {len(exp)} rows, now it prints out {len(output)} rows, the print out was\n{acual}',
                )
                for i in range(len(exp)):
                    self.assertEqual(
                        exp[i],
                        output[i].strip(),
                        f'The row {i+1} printed out after calling shape({y_height}, "{y_character}", {a_height}, "{a_character}") should be:\n{exp[i]}\nbut it is:\n{output[i]}\nWhole print out of the function was\n{output_all}',
                    )
            ...
            for line in f:
                ...
                if in_function:
                    if "  print" in line:
                        self.assertTrue(
                            False,
                            f"Function shape should not use print commands for printing, so following row is not allowed in your code\n{line}",
                        )
