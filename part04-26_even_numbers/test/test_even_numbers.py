...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as even_numbers(my_list: list)",
            )
        try:
            from src.even_numbers import even_numbers
            ...
            self.assertTrue(
                False, "Does following function call work?\neven_numbers([1,2])"
            )
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == list,
            f"Function {function} does not return list with parameter values [1,2].",
        )
        ...
        for test_case in test_cases:
            with patch(
                ...
                self.assertEqual(
                    sorted(correct),
                    sorted(test_result),
                    f"The result {test_result} does not match with the expected answer {correct} when calling even_numbers({test_case2})",
                )
                self.assertEqual(
                    test_case,
                    test_case2,
    ...
    def test_4_numbers_2(self):
        test_cases = {
            (99, 100, 101, 100, 99, 100, 101): [100, 100, 100],
