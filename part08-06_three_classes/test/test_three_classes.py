...
"""
    ...
    def test2_costructors(self):
        try:
            ...
            self.assertTrue(
                False,
                'Constructor call Checklist("list", [])' + f" threw an error: {e}",
            )
        try:
            from src.three_classes import Customer
            ...
            self.assertTrue(
                False,
                'Constructor call Customer("Customer",1,1.0)' + f" threw an error: {e}",
            )
        ...
        try:
            ...
            self.assertTrue(
                False,
                'Constructor call Cable("Cable",1.0,1.True)' + f" threw an error: {e}",
            )
        ...
        with patch(
            ...
            for attr in attributes:
                ...
                self.assertTrue(
                    ...
                    f"The object should have an attribute {attr},"
            ...
            for attr in attributes:
                ...
                self.assertTrue(
                    ref.has_attribute(attr),
                    f"The object shoul have an attribute {attr},"
                    + f"\nnow attributes are\n{f(ref.list_attributes(True))}\nwhen constructor was called with"
    ...
    def test4_test_checklist(self):
        test_cases = [
            ("Bills", ["Remember rent", "Remember phone bill"]),
        ...
        for test_case in test_cases:
            with patch(
                "builtins.input",
                side_effect=[AssertionError("Input was not excpected")],
            ):
                reload_module(self.module)
                from src.three_classes import Checklist
    ...
    def test5_test_customer(self):
        test_cases = [
            ("Caleb Customer", 1424.50, 10),
            ("Ibrahim Imagination", 550.0, 7),
            ("Mary Madeup", 240.25, 15),
        ]
