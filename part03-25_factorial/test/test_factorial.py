...
def get_correct(s: list) -> str:
    ...
    ss = "\n".join(
        [
            ...
            + str(i)
            + " is "
            + str(reduce(lambda a, b: a * b, range(1, i + 1)))
            for i in s
            if i > 0
        ]
    )
...
class FactorialTest(unittest.TestCase):
    ...
    def test_1_execution_ends(self):
        with patch(
            "builtins.input",
            side_effect=["1", "0", AssertionError("Input is asked too many times.")],
        ):
            try:
        ...
        with patch(
            "builtins.input",
            side_effect=["1", "-1", AssertionError("Input is asked too many times.")],
        ):
            try:
                reload_module(self.module)
            except:
        ...
        with patch(
            "builtins.input",
            side_effect=["1", "-451", AssertionError("Input is asked too many times.")],
        ):
            try:
                reload_module(self.module)
            except:
        ...
        for testcase in testcases:
            with patch("builtins.input", side_effect=testcase.split(" ")):
                ...
                self.assertTrue(
                    ...
                    "With the input {}, instead of {} rows, your program prints out {} rows:\n{}".format(
                        f(testcase), len_correct, len(output), output_all
                    ),
                )
