...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should contain function named as histogram(my_str: str)",
            )
        ...
        try:
            ...
            self.assertTrue(
                False, f"Make sure, that function can be called as follows:\n{koodi}"
            )
    ...
    def test_2_abba_works(self):
        ...
        with patch("builtins.input", side_effect=["2"] * 10):
            ...
            self.assertTrue(
                len(output_all) > 0, f"Calling {code} does not print out anything"
            )
            exp = ["a **", "b **"]
            self.assertEqual(
                len(exp),
                len(output),
                f"Calling {code} should print out {len(exp)} row,but it printed out {len(output)} rows. The print out was\n{output_all}",
            )
            for rivi in exp:
                self.assertTrue(
                    rivi in output,
                    f"Calling {code} should print out row \n{rivi}\nbut that row was not found. The print out was\n{output_all}",
                )
