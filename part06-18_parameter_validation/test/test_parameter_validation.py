...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your program should contain a function new_person(name: str, age: int)",
            )
        ...
        for test_case in test_cases:
            with patch(
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Function threw an error with input {test_case}, although the parameter values are valid!",
                    )
                except:
                    self.assertTrue(
                        False,
                        f"Function failed to execute with input {test_case} - check the program code!",
                    )
                self.assertEqual(
                    person,
                    test_case,
                    f"Function should return value {test_case} when input is {test_case} - now function returned {person}",
                )
        ...
        for test_case in test_cases:
            with patch(
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Function did not throw an error with input \n{test_case}\nalthough the parameters were invalid!",
                    )
                except ValueError:
                    pass
