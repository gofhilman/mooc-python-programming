...
class TemperaturesTest(unittest.TestCase):
    ...
    def test_1_zero(self):
        ...
        with patch("builtins.input", return_value=str(test_input)):
            ...
            self.assertFalse(
                output[-1].find("Brr! It's cold in here!") > -1,
                "Your program printed out 'Brr! It's cold in here!' even in a case where the temperature in degrees Celsius is not below zero.",
            )
            self.assertEqual(
                len(output),
                1,
                "Your program printed out more than one row when input was 32",
            )
            out = output[0]
            ...
            self.assertTrue(
                ...
                f"Your program should print out\n{e}\nwhen input is {32}, but now print out is\n{out}",
            )
            result = float(remove_extra_whitespace(out).split(" ")[-3])
            self.assertTrue(
                close(result, 0.0),
                "Your program did convertion of the temperature incorrectly: result should be 0.0, but print out of your program is "
                + output[0],
            )
    ...
    def test_2_positive(self):
        ...
        with patch("builtins.input", return_value=str(test_input)):
            ...
            self.assertEqual(
                len(output), 1, "Your program printed out more than one row"
            )
            out = output[0]
            result = float(remove_extra_whitespace(out).split(" ")[-3])
            self.assertTrue(
                close(result, correct),
                "Your program did not convert the temperature correctly: result with input {} should be {}, but now print out is {}".format(
                    test_input, correct, output[0]
                ),
            )
    ...
    def test_3_negative(self):
        ...
        with patch("builtins.input", return_value=str(test_input)):
            ...
            self.assertTrue(
                len(output) > 1,
                f"Your program did not print out message 'Brr! It's cold in here!' in a case where the temperature is below zero. Make sure that this print out happens with input {test_input}",
            )
            self.assertTrue(
                output[1].find("Brr! It's cold in here!") > -1,
                f"Your program did not print out message 'Brr! It's cold in here!' in a case where the temperature is below zero. Make sure that this print out happens with input {test_input}",
