        ...
        for test_case in values:
            with patch("builtins.input", side_effect=test_case):
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure that your program works correctly with the input {test_case}",
                    )
                out = get_stdout()
                output = out.split("\n")
                ...
                self.assertTrue(
                    len(out) > 0,
                    "Your program does not print out anything with the inputs {}".format(
                        test_case
                    ),
                )
                self.assertTrue(
                    len(output) == 1,
                    f"Your program should print out only one row in addtion to asking for the inputs from the user, your program's print out is now in {len(output)} rows.",
                )
                self.assertTrue(
                    sanitize(corr) == sanitize(out),
                    f"The print out is incorrect with the inputs {test_case}: your program's print out is\n{out}\nwhen correct print out is\n{corr}",
                )
