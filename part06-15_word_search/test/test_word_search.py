...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your program should contain a function find_words(search_term: str)",
            )
        ...
        try:
            ...
            self.assertTrue(
                False,
                'Plese ensure that the following function call works: find_words("cat")',
            )
        taip = str(type(val)).replace("<class '", "").replace("'>", "")
        self.assertTrue(
            type(val) == list,
            f'Function find_words("cat") should return a list, now it returns {val} which is of type {taip}.',
        )
    ...
    def test_2_search_no_special_chars(self):
        ...
        try:
            ...
            self.assertTrue(
                False, f"There was an error with search term {test_case}: {ioe}"
            )

        self.assertTrue(
            len(data) == len(correct),
        ...
        self.assertEqual(
            data,
            correct,
            f"Search term {test_case} should return lines \n{format(correct)} \nbut the function returned lines \n{format(data)}",
        )
