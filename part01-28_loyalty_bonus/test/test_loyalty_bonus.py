...
class LoyaltyBonusTest(unittest.TestCase):
    ...
    def check(self, output, points, correct_points):
        ...
        self.assertTrue(
            ...
            "With input {} correct amount of points {} is not found in output {}".format(
                points, correct_points, output
            ),
        )
    ...
    def test_under_99(self):
        ...
        with patch("builtins.input", return_value=str(points)):
            ...
            self.assertTrue(
                len(output_list) == 2,
                "With input {} instead of two rows, your program did print out {} rows".format(
                    points, len(output_list)
                ),
            )
            self.assertTrue(
                output.find("Your bonus is 10") > -1,
                "Row 'Your bonus is 10 %' was not found in output. Output was "
                + output,
            )
            self.assertFalse(
                output.find("bonus is 15") > -1,
                "Output contained incorrect bonus-%, 15 %:', print out was " + output,
            )
    ...
    def test_100(self):
        ...
        with patch("builtins.input", return_value=str(points)):
            ...
            self.assertTrue(
                output.find("Your bonus is 15") > -1,
                "Row 'Your bonus is 15 %' was not found in output. Output was "
                + output,
            )
            self.assertFalse(
                output.find("bonus is 10") > -1,
                "Output contained incorrect bonus-%, 10 %:', print out was " + output,
            )
            self.check(output, points, correct_points)
