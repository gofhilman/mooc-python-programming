...
"""
    ...
    def test_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Function hash_square is not found from your code or execution of the function is in infinite loop:\nTry: hash_square(1)",
            )
    ...
    def test_square_ok(self):
        for size in [2, 3, 5, 7, 10, 13, 25, 80]:
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f"Function call hash_square({size}) does not print out anything",
                )
                acual = "\n".join(output)
                self.assertEqual(
                    size,
                    len(output),
                    f"Function call hash_square({size}) should print out {size} rows, now it printed out {len(output)} rows, print out was\n{acual}",
                )
                for i in range(size):
                    self.assertEqual(
                        "#" * size,
                        output[i].strip(),
                        f"Each row which function call hash_square({size}) prints out, should be {'#'*size}, following row is incorrect\n{output[i]}\nwhole print out of the function was\n{acual}",
                    )
