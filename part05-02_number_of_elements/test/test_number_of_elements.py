...
class NumberOfElementsTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        with patch(
            "builtins.input",
            side_effect=[AssertionError("Asking input from the user was not expected")],
        ):
...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as count_matching_elements(my_matrix: list, element: int)",
            )
        try:
            from src.number_of_elements import count_matching_elements
    ...
    def test_2_type_of_return_value(self):
        ...
        self.assertTrue(
            type(val) == int,
            f"Function {function} does not return value of integer type with parameter values [[1]], 1.",
        )

    def test_3_matrices(self):
        test_cases = (
            ([[1, 2, 3], [2, 3, 1], [4, 5, 6]], 2),
            ([[1, 5, 5, 3], [2, 5, 2, 5], [0, 0, 2, 5]], 5),
            ([[1, 2, 3, 4], [2, 3, 4, 5], [3, 4, 6, 5]], 6),
        )
        for test_case in test_cases:
            with patch(
                "builtins.input",
                side_effect=[
                    AssertionError("Asking input from the user was not expected")
                ],
            ):
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure that the function works when matrice is {test_case[0]} and element is {test_case[1]}",
                    )

                self.assertEqual(
                    correct,
                    test_result,
                    f"The result {test_result} does not match with the model solution {correct} when matrice is {test_case[0]} and element is {test_case[1]}",
                )
                self.assertEqual(
                    test_case[0],
                    test_case2,
                    f"Function should not change the original list. The value should should be {test_case2} but it is {test_case}.",
                )
        ...
        for i in range(5):
            ...
            with patch(
                ...
                try:
                    ...
                    self.assertTrue(
                        False,
                        f"Make sure that the function works when matrice is {test_case} and element is {val}",
                    )

                self.assertEqual(
                    correct,
                    test_result,
                    f"The result {test_result} does not match with the model solution {correct} when matrice is  {test_case} and element is {val}.",
                )
                self.assertEqual(
                    test_case,
                    test_case2,
