...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.assertTrue(
                False, f"Your code should contain function named as greatest_number"
            )
        try:
            from src.greatest_number import greatest_number
    ...
    def test_2_function_ok(self):
        for a, b, c in [
            ...
            with patch("builtins.input", side_effect=["2"] * 10):
                ...
                self.assertFalse(
                    ret == None,
                    f"Calling greatest_number({a}, {b}, {c}) should return {max([a, b, c])} now it does not return anything. Make sure that you use return command in your function",
                )
                self.assertEqual(
                    ret,
                    max([a, b, c]),
                    f"Calling greatest_number({a}, {b}, {c}) should return {max([a, b, c])} now it returns {ret}",
                )
                self.assertFalse(
                    len(output_all) > 0,
                    f"Calling greatest_number({a}, {b}, {c}) should not print out anything, but it prints out\n{output_all}\nremove print commands inside function",
                )
