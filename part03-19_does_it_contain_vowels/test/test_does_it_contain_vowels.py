    ...
    def test_strings(self):
        ...
        for test_case in values:
            with patch("builtins.input", side_effect=[test_case]):
                ...
                cor = [
                    x + " found" if (x in test_case) else (x + " not found")
                    for x in "aeo"
                ]
                self.assertEqual(
                    len(output_list),
                    len(cor),
                    f"Your program should print out {len(cor)} row, now it prints out {len(output_list)} rows, when the input is: {test_case}",
                )
                r = 1
