    ...
    def test_1_small_symmetric(self):
        ...
        for word in words:
            with patch("builtins.input", side_effect=word[1:-1].split(",")):
                ...
                self.assertTrue(
                    len(output[0]) == w,
                    f"With the input {word} width of the rectangle should be {w}, it was {len(output[0])}. Your program's output:\n{output_all}",
                )
                self.assertTrue(
                    len(output) == h,
                    f"With the input {word} height of the rectangle should be {h}, it was {len(output)}. Your program's output:\n{output_all}",
                )
                self.assertTrue(
