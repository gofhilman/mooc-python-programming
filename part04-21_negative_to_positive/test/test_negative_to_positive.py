    ...
    def test_inputs(self):
        ...
        for test_case in values:
            with patch("builtins.input", side_effect=[test_case]):
                ...
                cor = [
                    str(x)
                    for x in range(-(int(test_case)), int(test_case) + 1)
                    if x != 0
                ]
...
block
            """
                ...
                self.assertTrue(
                    len(output) > 0,
                    f"Your program does not print out anything with the input {test_case}\n{mssage}",
                )
                self.assertEqual(
                    len(output_list),
                    len(cor),
                    f"In addition to asking for the inputs from the user, your program should print out {len(cor)} rows, now it prints out {len(output_list)} rows when the input is {test_case}",
                )
                r = 1
