    ...
    def test_short_words(self):
        ...
        for word in words:
            with patch("builtins.input", return_value=word):
                ...
                self.assertFalse(
                    len(output_all) == 0,
                    "Your program does not print out anything with the input " + word,
                )
                self.assertTrue(
                    len(output) == 1,
                    "With the input {}, instead of {} rows, your program's print out is in {} rows".format(
                        word, 1, len(output)
                    ),
                )
                self.assertTrue(
                    output_all.count("*") == 20 - len(word),
                    "Instead of correct amount of stars ({}) your program prints out {} stars, with the input {}: \n{}".format(
                        (20 - len(word)), output_all.count("*"), word, output_all
                    ),
                )
                self.assertTrue(
                    len(output_all) == 20,
                    "Instead of 20 characters, length of your print out is {} characters. Print out was:\n{}\nExpected print out was\n{}".format(
                        len(output_all), output_all, correct
                    ),
                )
                self.assertTrue(
                    outputs_equal(output_all, correct),
                    "Your program's print out\n{}\ndoes not match with the correct print out\n{}\nwith the input {}".format(
                        output_all, correct, word
                    ),
                )
