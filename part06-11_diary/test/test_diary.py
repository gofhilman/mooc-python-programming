...
"""
            ...
            self.assertTrue(
                correct in output,
                f"Program should output {correct}, now the output is \n{output}",
            )
    ...
    def test_3_clear_add_2_lines_and_exit(self):
        ...
        with patch(
            "builtins.input",
            side_effect=["1", "Today was hot", "1", "I learned more Python", "0"],
        ):
            reload_module(self.module)
            output = get_stdout()

            input_values = "\n".join(
                ["1", "Today was hot", "1", "I learned more Python", "0"]
            )
            ...
            self.assertTrue(
                len(content) == len(correct),
                f"If file was empty at first, with input\n{input_values}\nthe file should now contain 2 lines:\n{format(correct)}\nThe content is\n{format(content)}",
            )
            self.assertEqual(
                content,
                correct,
                f"If file was empty at first, with input value\n{input_values}\nthe content of the file should be \n{format(correct)}\nbut instead it is\n{format(content)}",
            )
    ...
    def test_4_output_content(self):
        with patch("builtins.input", side_effect=["2", "0"]):
            ...
            output = [
                x.strip()
                for x in output
                if "1 - " not in x and "ye now" not in x and not "tries" in x
            ]
