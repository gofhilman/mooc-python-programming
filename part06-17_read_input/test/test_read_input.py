...
"""
    ...
    def test_1_function_exists(self):
        try:
            ...
            self.fail(
                "Your code should contain function read_input(prompt: str, lower_limit: int, upper_limit: int)"
            )
    ...
    def test_2_ok_numbers(self):
        with patch("builtins.input", side_effect=["7"]):
            ...
            try:
                ...
                self.fail(
                    'Ensure that the function can be called like this: read_input("Enter a number:", 2, 10)'
                )

            self.assertEqual(
                result, 7, "Function should return value 7 when user enters 7"
            )
    ...
    def test_3_too_small1(self):
        with patch("builtins.input", side_effect=["4", "6"]):
            ...
            output = get_stdout()
            self.assertTrue(
                "You must type in an integer between 5 and 10" in output,
    ...
    def test_5_no_numbers(self):
        with patch("builtins.input", side_effect=["one", "two", "three", "100"]):
            ...
            try:
                ...
                self.fail(
                    "Ensure that your program can be execute with input values\none\ntwo\nthree\n100"
                )
            output = get_stdout()
            self.assertTrue(
                "You must type in an integer between 95 and 105" in output,
            ...
            self.assertEqual(
                result,
                100,
                f"Function should return value 100 when user enters 100 and the function is called with arguments ('Enter a number', 95, 105). Now the function returns {result}",
            )
