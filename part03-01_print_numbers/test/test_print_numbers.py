...
class PrintNumbersTest(unittest.TestCase):
    ...
    def test_print_numbers(self):
        ...
        self.assertEqual(
            len(output_list),
            15,
            f"Your program is expected to print out 15 rows, now print out is in {len(output_list)} rows.",
        )
        r = 1
        for l1, l2 in zip(cor, output_list):
            self.assertEqual(
                l1,
                l2,
                f"Print out in row {r+1} does not match with the model solution: program should print out {l1}, but it prints out {l2}.",
            )
