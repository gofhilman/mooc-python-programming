...
"""
    ...
    def test_funktio_olemassa(self):
        try:
            ...
            self.assertTrue(
                False,
                f"Your code should contain function named as chessboard, which can be called as follows: chessboard(2)",
            )
    ...
    def test_board(self):
        with patch("builtins.input", side_effect=["2"] * 100):
            for size in range(3, 60):
                ...
                self.assertTrue(
                    len(output_all) > 0,
                    f"Function call chessboard({size}) does not print out anything",
                )
                acual = "\n".join(output)
                self.assertEqual(
                    size,
                    len(output),
                    f"Function call chessboard({size}) should print out {size} rows, now it printed out {len(output)} rows, print out was\n{acual}",
                )
                for i in range(size):
                    ...
                    self.assertEqual(
                        row,
                        output[i].strip(),
                        f"Row {i}, which function call chessboard({size}) prints out, should be {row}, now it is\n{output[i]}\nwhole print out of the function was\n{acual}",
                    )
