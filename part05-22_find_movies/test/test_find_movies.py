...
"""
    ...
    def test_1_funktio_olemassa(self):
        try:
            ...
            self.assertTrue(
                False,
                "Your code should contain function named as find_movies(database: list, search_term: str)",
            )
        try:
    ...
    def test_2_type_of_return_value(self):
        find_movies = load(exercise, function, "en")
        val = find_movies(
            [{"name": "aa", "director": "", "year": 1, "runtime": 1}], "a"
        )
        taip = str(type(val)).replace("<class '", "").replace("'>", "")
        self.assertTrue(
            type(val) == list,
            f"Function {function} should return list, now it returns value {val} which is {taip} type",
        )
    ...
    def test_3_movies_1(self):
        ...
        for tc in test_cases:
            movielist.append(
                {x: y for x, y in zip(("name", "director", "year", "runtime"), tc)}
            )
    ...
    def test_4_movies_2(self):
        test_cases = (
            ("The Birds 4", "James Hitchcock", 2003, 119),
